...
Page.WebHooks = class WebHooks extends Page.PageUtils {
	...
	get_web_hook_dependants(hook) {
		// get lists of things that depend on the current web hook
		var self = this;
		var flows = {};
		var events = {};
		var cats = {};
		...
		if (!hook) hook = this.web_hook;
		
		app.events.forEach( function(event) {
			(event.actions || []).forEach( function(action) {
				if ((action.type == 'web_hook') && (action.web_hook == hook.id)) {
			...
			if (event.workflow && event.workflow.nodes) event.workflow.nodes.forEach( function(node) {
				if ((node.type == 'action') && node.data && node.data.web_hook && (node.data.web_hook == hook.id)) flows[ event.id ] = 1;
			} );
		} ); // foreach event
		
		app.alerts.forEach( function(alert) {
			if (find_object(alert.actions || [], { type: 'web_hook', web_hook: hook.id })) alerts[ alert.id ] = 1;
		} );
		
		// categories
		app.categories.forEach( function(cat) {
			(cat.actions || []).forEach( function(action) {
				if ((action.type == 'web_hook') && (action.web_hook == hook.id)) cats[ cat.id ] = 1;
			} );
		} );
		...
		app.groups.forEach( function(group) {
			(group.alert_actions || []).forEach( function(action) {
				if ((action.type == 'web_hook') && (action.web_hook == hook.id)) groups[ group.id ] = 1;
			} );
		} );
	...
	show_delete_web_hook_dialog() {
		...
		var deps = this.get_web_hook_dependants();
		if (!deps) {
		...
		// show user a summary of hook's dependants
		var title = 'Delete Web Hook';
		var md = '';
		var html = '';
		
		md += `Are you sure you want to **permanently delete** the web hook &ldquo;${this.web_hook.title}&rdquo;?\n\nPlease note the following dependants that use the hook:\n`;
