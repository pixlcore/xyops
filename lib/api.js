...
class API {
	...
	requirePrivilege(user, priv_id, callback) {
		...
		if (user.key) {
			return this.doError('api', `API Key "${user.title}" does not have the "${priv_def.title}" privilege required to perform this action.`, callback);
		}
		else {
			return this.doError('user', `You do not have the "${priv_def.title}" privilege required to perform this action.`, callback);
		}
	}
	...
	loadSession(args, callback) {
		...
		this.storage.listFind( 'global/api_keys', { key: api_key }, function(err, item) {
			if (err || !item) return callback(new Error("Invalid API Key"), null, null);
