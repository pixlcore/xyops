	...
	execSchedulerPlugin(plugin, items, launch_jobs, callback) {
		...
		if (plugin.uid && (plugin.uid != 0)) {
			...
			if (user_info) {
				...
				var err_msg = "Could not determine user information for: " + plugin.uid;
				if (launch_jobs) {
		...
		if (plugin.gid && (plugin.gid != 0)) {
			...
			if (grp_info) {
				...
				var err_msg = "Could not determine group information for: " + plugin.gid;
				if (launch_jobs) {
					this.logError('scheduler', err_msg, { plugin: plugin.id } );
					return callback();
				}
		...
		var child = cp.exec( child_cmd, child_opts, function(err, stdout, stderr) {
			...
			if (err) {
				var err_msg = "Failed to launch Trigger Plugin: " + plugin.id + ": " + err;
				if (launch_jobs) {
