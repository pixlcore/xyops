...
class Scheduler {
	...
	setupScheduler() {
		...
		// setup queue for launching jobs
		this.launchQueue = async.queue( this.dequeueLaunch.bind(this), 1 );
		...
		// check startup jobs in another thread, to ensure everything is 100% ready to go
		setTimeout( function() { self.checkStartupTriggers(); }, 250 );
	}
	
	enqueueLaunch(job, callback) {
		// enqueue job for launch as soon as possible
		if (!callback) callback = noop;
		this.launchQueue.push({ job, callback });
	}
	
	dequeueLaunch(item, callback) {
		// launch one job and sleep to delay the next queued item
		if (this.shut) {
			item.callback( new Error("Conductor is shutting down.") );
			return process.nextTick(callback);
		}
		
		this.launchJob(item.job, item.callback);
		
		setTimeout( function() { callback(); }, this.config.get('scheduler_stagger_ms') || 50 );
	}
	
	checkStartupTriggers() {
		// fire all events with active startup triggers
		var self = this;
		if (!this.master || this.shut) return;
		
		if (!this.getState('scheduler/enabled')) {
			this.logScheduler(5, "Scheduler is disabled, skipping startup trigger check");
			return;
		}
		
		var min_uptime = Tools.getSecondsFromText( this.config.get('min_startup_trigger_uptime') || '5 minutes' );
		if (process.uptime() > min_uptime) {
			this.logScheduler(5, `Process uptime is greater than ${min_uptime} seconds, skipping startup trigger check`);
			return;
		}
		
		if (this.processedBackgroundLog) {
			this.logScheduler(5, `Background command was executed, skipping startup trigger check`);
			return;
		}
		...
		this.events.forEach( function(event) {
			...
			// make sure event has enabled startup trigger
			var trigger = Tools.findObject(event.triggers, { enabled: true, type: 'startup' });
			if (!trigger) return;
			...
			// allow trigger to pass along data
			if (trigger.data) {
				if (!job.input) job.input = {};
				if (!job.input.data) job.input.data = {};
				
				if (Array.isArray(trigger.data)) job.input.data.items = Tools.copyHash(trigger.data, true);
				else Tools.mergeHashInto( job.input.data, Tools.copyHash(trigger.data, true) );
			}
			
			// allow trigger to pass along params
			if (trigger.params) {
				if (!job.params) job.params = {};
				Tools.mergeHashInto( job.params, Tools.copyHash(trigger.params, true) );
			}
			
			// allow trigger to apply tags
			if (trigger.tags) {
				if (!job.tags) job.tags = [];
				job.tags = job.tags.concat( trigger.tags );
			}
			
			self.logScheduler(5, "Running event at startup: " + event.title, job);
			self.enqueueLaunch(job, function(err, id) {
				if (err) self.logError('scheduler', "Failed to launch startup event: " + (err.message || err));
				else self.logScheduler(5, "Startup job was started: " + id);
			});
		}); // foreach event
	}
	...
	schedulerMinuteTick(dargs) {
		...
		this.events.forEach( function(event) {
			...
			var nth_trigger = Tools.findObject( triggers, { type: 'nth' } );
			
			var cursor = catch_up ? (self.getState('events/' + event.id + '/cursor') || (now - 60)) : (now - 60);
			var nth_counter = nth_trigger ? (self.getState('events/' + event.id + '/nth') || 0) : 0;
			var nth_changed = false;
			...
			while (cursor < now) {
				...
				if (nth_trigger) {
					nth_counter--;
					if (nth_counter <= 0) nth_counter = nth_trigger.every;
				...
				chosen_triggers.forEach( function(chosen_trigger) {
					// we're go for launch!
					var job = Tools.copyHash(event, true);
					job.now = cursor;
					job.source = 'scheduler';
					if (chosen_trigger.type != 'schedule') job.stype = chosen_trigger.type; // single, interval, etc.
					...
					// allow trigger to pass along data
					if (chosen_trigger.data) {
						if (!job.input) job.input = {};
						if (!job.input.data) job.input.data = {};
						
						if (Array.isArray(chosen_trigger.data)) job.input.data.items = Tools.copyHash(chosen_trigger.data, true);
						else Tools.mergeHashInto( job.input.data, Tools.copyHash(chosen_trigger.data, true) );
					}
					
					// allow trigger to pass along params
					if (chosen_trigger.params) {
						if (!job.params) job.params = {};
						Tools.mergeHashInto( job.params, Tools.copyHash(chosen_trigger.params, true) );
					}
					
					// allow trigger to apply tags
					if (chosen_trigger.tags) {
						if (!job.tags) job.tags = [];
						job.tags = job.tags.concat( chosen_trigger.tags );
					}
					...
					// optional quiet flags
					if (quiet) {
						if (quiet.invisible) job.invisible = quiet.invisible;
						if (quiet.ephemeral) job.ephemeral = quiet.ephemeral;
					}
					...
					if (plugin_trigger) {
					...
					else if (precision && precision.seconds && precision.seconds.length) {
						...
						precision.seconds.forEach( function(sec) {
							...
							self.enqueueLaunch( Tools.mergeHashes( Tools.copyHash(job, true), {
								now: cursor + sec,
								state: 'start_delay',
								until: cursor + sec
							} ) );
						...
						self.enqueueLaunch(job);
					}
				}); // foreach chosen trigger
			} // while cursor
			
			// update event cursor state if in catch-up mode
			if (catch_up) self.putState( 'events/' + event.id + '/cursor', cursor );
			if (nth_trigger && nth_changed) self.putState( 'events/' + event.id + '/nth', nth_counter );
	...
	handleSchedulerPluginJobs(deferred_jobs, callback) {
		...
		async.each( Object.values(plugins),
			function(items, callback) {
				...
				self.execSchedulerPlugin(plugin, items, true, callback);
			},
			function() {
	...
	execSchedulerPlugin(plugin, items, launch_jobs, callback) {
		...
		var puid = plugin.uid || this.config.getPath('default_plugin_credentials.scheduler.uid') || '';
		var pgid = plugin.gid || this.config.getPath('default_plugin_credentials.scheduler.gid') || '';
		...
		if (puid && (puid != 0)) {
			...
			if (user_info) {
				...
				var err_msg = "Could not determine user information for: " + puid;
				if (launch_jobs) {
					this.logError('scheduler', err_msg, { plugin: plugin.id } );
					return callback();
				}
				else return callback({ code: 1, description: err_msg });
			}
		}
		if (pgid && (pgid != 0)) {
			...
			if (grp_info) {
				...
				var err_msg = "Could not determine group information for: " + pgid;
				if (launch_jobs) {
					this.logError('scheduler', err_msg, { plugin: plugin.id } );
					return callback();
				}
		...
		var child = cp.exec( child_cmd, child_opts, function(err, stdout, stderr) {
			...
			if (err) {
				var err_msg = "Failed to launch Trigger Plugin: " + plugin.id + ": " + String(err).replace(/\r\n/g, "\n").trim().split(/\n/).shift();
				if (launch_jobs) {
					self.logError('scheduler', err_msg, { cmd: child_cmd, stdout, stderr });
					self.logActivity('warning', {
						description: err_msg,
						details: "**Raw Output:**\n\n```\n" + String(stdout + "\n" + stderr).trim() + "\n```\n"
					});
					return callback();
				}
				else return callback({ code: 1, description: err_msg, child_cmd, stdout, stderr });
			}
			if (!json || !json.xy || !json.items || !json.items.length) {
				var err_msg = "Unexpected result from Trigger Plugin: " + plugin.id + " (missing required JSON properties)";
				if (launch_jobs) {
					self.logError('scheduler', err_msg, { cmd: child_cmd, stdout, stderr });
					self.logActivity('warning', {
						description: err_msg,
			...
			// if launch jobs is false (i.e. testing), return results here
			if (!launch_jobs) return callback({ code: 0, child_cmd, stdout, stderr, data: json });
			...
			async.eachOfSeries( json.items, 
				function(result, idx, callback) {
					...
					// cancel invisible mode for all jobs launched via trigger plugins
					// (unless plugin itself returns an invisible result property)
					if (result.invisible) job.invisible = true;
					else delete job.invisible;
		...
		child.stdin.write( JSON.stringify({ 
			...
			secrets: sec,
			base_url: base_url,
			active_jobs: Object.values(this.activeJobs).filter( job => job.state != 'queued' )
		}) + "\n" );
