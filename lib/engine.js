...
class Orchestra extends Component {
	...
	afterUserChange(action, args) {
		...
		if ((action == 'user_create') || (action == 'user_update')) {
			// send redacted user update to all connected users
			self.doUserBroadcastAll('single', { list: 'users', item: redacted_user });
		}
		else if (action == 'user_delete') {
			self.doUserBroadcastAll('single', { list: 'users', item: redacted_user, delete: true });
		}
	}
