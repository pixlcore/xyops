...
class Satellite {
	...
	api_get_satellite_releases(args, callback) {
		...
		// custom ttl for satellite releases (so we don't hit GH too often)
		this.setCacheResponse(args, sat.cache_ttl || 3600);
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (sat.disable_version_checks) {
				return callback({ code: 0, releases: ['latest'] });
			}
			
			self.logDebug(9, "Fetching satellite release list: " + sat.list_url );
