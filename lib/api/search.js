...
class Search {
	...
	api_marketplace(args, callback) {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (!marketplace.enabled) {
				return self.doError('marketplace', "The marketplace system is not enabled.  Please enable it in your xyOps configuration and try again.", callback);
			}
			...
			var finish = function(metadata) {
				if (params.id) {
					...
					if (params.readme) {
						...
						self.request.get( url, { retries: 8, retryDelay: 50 }, function(err, resp, data, perf) {
							if (err) return self.doError( 'marketplace', "Failed to fetch logo: " + err, callback );
							// cache for a long time because url includes the version
				...
				// pre-cache all installed market plugins, for quick lookups in filtering below
				var installed = {};
				self.plugins.forEach( function(plugin) {
					if (plugin.marketplace) installed[ plugin.marketplace.id ] = plugin.marketplace.version;
				} );
				...
				var rows = metadata.rows.filter( function(row) {
					...
					if (params.status) {
						if ((params.status === 'installed') && !installed[row.id]) return false;
						if ((params.status === 'not') && installed[row.id]) return false;
					}
	...
	ws_search_job_files(args) {
		...
		var processCustom = function(job, path, opts) {
			...
			self.logDebug(10, "Searching custom job property: " + job.id + ": " + path);
