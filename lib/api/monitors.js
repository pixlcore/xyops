...
class Monitors {
	...
	api_get_monitor(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!monitor) return self.doError('notfound', "Failed to locate monitor: " + params.id, callback);
	...
	api_create_monitor(args, callback) {
		...
		if (!this.requireParams(params, {
			...
			title: /\S/,
			source: /\S/,
			data_type: /^(integer|float|bytes|seconds|milliseconds)$/
		}, callback)) return;
	...
	api_update_monitor(args, callback) {
		...
		if (params.data_type && !String(params.data_type).match(/^(integer|float|bytes|seconds|milliseconds)$/)) {
			return this.doError('monitor', "Invalid data type: " + params.data_type, callback);
		}
