...
class FileManagement {
	...
	api_upload_files(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var storage_key_prefix = 'files/' + (user.username || user.id);
	...
	api_upload_job_file(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			// JH 2026-02-28 Skipping this check, as files may come in late, after job has died
			...
			var storage_key_prefix = 'files/jobs/' + params.id + '/' + Tools.generateUniqueBase64(32);
			var temp_file = file.path;
			...
			self.logDebug(7, "Uploading job file", { file, storage_key });
	...
	api_delete_job_file(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			self.unbase.get( 'jobs', params.id, function(err, job) {
				...
				// support both input (limited) and output files
				var file = Tools.findObject(job.files || [], { path: params.path });
				
				if (file) {
					// output file, easy
					Tools.deleteObject(job.files, { path: params.path });
				}
				else if (job.input && job.input.files) {
					// possibly input file, not so easy
					file = Tools.findObject(job.input.files, { path: params.path });
					if (!file) return self.doError('job', "Job file not found: " + Path.basename(params.path), callback);
					
					if (file.bucket) return self.doError('job', "Bucket files cannot be deleted from job: " + Path.basename(params.path), callback);
					if (file.ticket) return self.doError('job', "Ticket files cannot be deleted from job: " + Path.basename(params.path), callback);
					
					Tools.deleteObject(job.input.files, { path: params.path });
				}
				else {
				...
				self.storage.delete( params.path, function(err) {
					if (err && (err.code != 'NoSuchKey')) {
						return self.doError('job', "Failed to delete job file: " + params.path + ": " + err, callback);
					}
