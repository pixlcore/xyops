...
class Tickets {
		...
		this.loadSession(args, function (err, session, user) {
			...
			if (params.id) {
				self.unbase.get( 'tickets', params.id, function(err, record) {
					if (err) return self.doError('notfound', "Failed to load ticket: " + params.id + ": " + (err.message || err), callback );
				...
				self.unbase.search( 'tickets', 'num:' + params.num, { offset:0, limit:1 }, function(err, results) {
					...
					if (!results || !results.records || !results.records[0] || !results.records[0].num) {
						return self.doError( 'notfound', "Ticket not found: " + params.num, callback );
					}
	...
	api_search_tickets(args, callback) {
		...
		var compact = (params.compact == 1) || (params.compact == "true");
		...
		this.loadSession(args, function(err, session, user) {
			...
			function(err) {
				...
				self.unbase.insert( { index: 'tickets', id: params.id, data: params, fast: true }, function(err) {
					...
					// send notification immediately for new tickets
					if ((params.status != 'draft') && self.config.getPath('tickets.email_enabled')) {
						self.processDelayedTicketChange({ id: params.id, changes: params.changes });
					}
		...
		this.loadSession(args, function(err, session, user) {
			...
			function(err) {
				...
				self.unbase.update( { index: 'tickets', id: params.id, data: params, fast: true }, function(err, ticket) {
					...
					// buffer changes for delayed notification
	...
	requireValidTicketParams(params, callback) {
		...
		if (params.assignees && !Array.isArray(params.assignees)) return this.doError('ticket', "Malformed ticket parameter: assignees", callback);
		...
		if (params.due && (typeof(params.due) == 'string')) {
			// convert deltas like "7 days" to seconds
			params.due = Tools.getSecondsFromText(params.due);
		}
		if (params.due && (typeof(params.due) == 'number') && (params.due < 315360000)) {
			// appears to be an offset, so add current time and normalize
			params.due = Tools.normalizeTime( Tools.timeNow(true) + params.due, { hour: 0, min: 0, sec: 0 } );
		}
		...
		if (params.events && params.events.length) {
			for (var idx = 0, len = params.events.length; idx < len; idx++) {
				var stub = params.events[idx];
				
				var event = Tools.findObject(this.events, { id: stub.id });
				if (!event) return this.doError('ticket', "Unknown event: " + stub.id, callback);
				
				try { this.validateParamValues(stub.params, event.fields); }
				catch (err) { return this.doError('ticket', "Ticket Event: " + err.message, callback); }
			} // foreach event
		} // has events
