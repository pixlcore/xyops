		...
		this.loadSession(args, function(err, session, user) {
			...
			async.series([
				...
				function(callback) {
					...
					var storage_key_prefix = 'files/' + params.id + '/' + (user.username || user.id) + '/' + Tools.generateUniqueBase64(16);
					params.files = [];
	...
	api_update_ticket(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			function(err) {
				...
				self.unbase.update( { index: 'tickets', id: params.id, data: params, fast: true }, function(err) {
					self.storage.unlock( lock_key );
					if (err) {
