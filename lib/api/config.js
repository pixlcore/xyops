...
class Configuration {
	...
	api_config(args, callback) {
		...
		if (args.query.code) {
			...
			var resp = { ...args.query };
			resp.host = 1; // bypass master check
			resp.version = this.server.__version; // show version on error page
			resp.config = this.config.get('client');
		...
		if (this.master) {
			resp.config = Tools.mergeHashes( this.config.get('client'), {
				...
				default_user_privileges: this.getDefaultUserPrivileges(),
		...
		if (args.query.format === 'json') callback(resp);
		else callback( "200 OK", { 'Content-Type': "text/javascript" }, 'app.receiveConfig(' + JSON.stringify(resp) + ');' );
	}
	...
	api_get_multiple(args, callback) {
		// fetch multiple in-memory lists at once
		var self = this;
		var params = Tools.mergeHashes( args.params, args.query );
		var list_names = this.config.getPath('ui.globalMemoryLists');
		if (!this.requireMaster(args, callback)) return;
		
		if (!params.lists) {
			return this.doError('admin', "Missing required request property: lists", callback);
		}
		if ((typeof(params.lists) == 'string') && (params.lists === "all")) {
			params.lists = list_names;
		}
		else {
			if (typeof(params.lists) == 'string') params.lists = params.lists.split(/\,\s*/);
			if (!Array.isArray(params.lists)) {
				return this.doError('admin', "Invalid request (lists must be an array).", callback);
			}
			
			// make sure all entries are valid lists
			if (!params.lists.every( name => list_names.includes(name) )) {
				return this.doError('admin', "One or more values are not valid lists.", callback);
			}
		}
		...
		this.loadSession(args, function(err, session, user) {
			...
			params.lists.forEach( function(name) {
				resp[name] = self[name];
			} );
			
			if (params.jobs) {
				resp.jobs = self.getActiveJobs();
			}
			if (params.state) {
				resp.state = Tools.copyHashRemoveKeys( self.state, { dirty: 1 } );
			}
			if (params.servers) {
				resp.servers = self.servers;
			}
			if (params.serverCache) {
				resp.serverCache = self.serverCache;
			}
