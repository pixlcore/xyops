	...
	requireValidPluginData(params, callback) {
		...
		if (params.params) {
			...
			for (var idx = 0, len = params.params.length; idx < len; idx++) {
				...
				if (param.title && (typeof(param.title) != 'string')) return this.doError('api', err_prefix + ": Title must be a string", callback);
				if (param.title && param.title.match(/[<>]/)) return this.doError('api', err_prefix + ": Title contains illegal characters", callback);
			}
		} // params.params
