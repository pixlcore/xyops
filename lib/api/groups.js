...
class Groups {
	...
	api_get_group(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (!group) return self.doError('notfound', "Failed to locate group: " + params.id, callback);
	...
	api_create_group(args, callback) {
		...
		catch (err) { return this.doError('group', "Invalid hostname match: " + err, callback); }
	...
	api_watch_group(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var group = Tools.findObject( self.groups, { id: params.id } );
			if (!group) return self.doError( 'watch', "Unknown group: " + params.id, callback );
