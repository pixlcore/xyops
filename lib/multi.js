...
class Multi {
	...
	becomeMaster() {
		...
		async.series(
			...
			function(err) {
				...
				if (!self.server.debug) self.checkStartupLogFiles();
			}
		); // async.series
	}
	...
	multiShutdown() {
		...
		if (this.master) {
			var recovery_file = Path.join( this.config.get('log_dir'), "_recovery.json" );
			var recovery_data = {};
			['activeJobs', 'jobDetails', 'activeAlerts', 'transferTokens', 'serverCache'].forEach( function(key) { recovery_data[key] = self[key]; } );
