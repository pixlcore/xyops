...
class Multi {
	...
	multiSetup() {
		...
		this.logDebug(2, "Initializing multi-server management system", {
			id: this.hostID
		});
		...
		if (this.config.get('master')) {
			...
			this.logDebug(1, "No master data found, registering ourselves (first run)");
	...
	api_master_command(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			if (params.host == self.hostID) {
				...
				self.logDebug(3, "Running master command on self", params);
				...
				if (peer.socket && peer.socket.connected) {
					self.logDebug(3, "Removing master server from cluster", params);
				...
				self.logDebug(3, "Sending master command to peer", params);
	...
	doMasterCommand(socket, data) {
		...
		var self = this;
		this.logDebug(3, "Running local master command", data);
		...
		if (this.server.debug) {
			this.logDebug(9, "Debug mode detected, performing local shutdown");
	...
	electMaster() {
		...
		this.logDebug(5, "Holding master election, contacting all candidates");
		...
		async.each(
			...
			function(host, callback) {
				self.logDebug(6, "Contacting server: " + host);
				...
				self.request.json( url, data, opts, function(err, resp, json, perf) {
					if (err) {
						self.logDebug(4, "Failed to contact candidate: " + host + ": " + err);
					}
					else if (json.code) {
						self.logDebug(4, "Error from candidate: " + host + ": " + json.description);
					}
					else if (json.master) {
						found_master = true;
						self.logDebug(3, "Found master server: " + host + " (and we have registered)");
					}
					else {
						online.push(host);
						self.logDebug(4, "Candidate is online: " + host);
					}
					callback();
				});
			},
			function() {
				...
				if (winner == self.hostID) {
					// we won!
					self.logDebug(3, "We are the highest ranking candidate");
					self.becomeMaster();
				}
				else {
					self.logDebug(3, "Candidate " + winner + " outranks us, so stepping aside.");
	...
	becomeMaster() {
		...
		this.logDebug(1, "We are becoming master", {
			id: this.hostID
		});
		...
		// init monitoring system (performa)
		...
		async.series(
			[
				...
				function(callback) {
					var keys = ['groups', 'plugins', 'categories', 'events', 'channels', 'monitors', 'alerts', 'commands', 'tags'];
	...
	doMasterCommitStart(socket, data) {
		...
		fs.writeFile( file, data.contents, function(err) {
			...
			else self.logDebug(9, "(Network Commit Start) Wrote local commit rollback log: " + file);
		});
	}
	...
	doMasterCommitEnd(socket, data) {
		...
		fs.unlink(file, function(err) {
			...
			else self.logDebug(9, "(Network Commit End) Deleted local commit rollback log: " + file);
		});
	}
	...
	addNewPeer(host) {
		...
		var self = this;
		this.logDebug(3, "Adding new peer server: " + host);
	...
	removePeer(host) {
		...
		this.logDebug(3, "Removing peer server: " + host);
	...
	receiveNewMaster(socket, data) {
		...
		this.logDebug(1, "We are now a backup server under master: " + data.host);
	...
	saveMasterData(callback) {
		...
		fs.writeFile( this.masterListFile, JSON.stringify(this.masterData, null, "\t") + "\n", function(err) {
			if (err) self.logError('multi', "Failed to write master data: " + this.masterListFile + ": " + err);
	...
	receiveMasterData(socket, data) {
		...
		this.logDebug(9, "Received update to masterData", data);
	...
	lostMaster(socket) {
		...
		this.logDebug(2, "Lost connection to master server: " + this.masterHost);
		...
		this.logDebug(3, "An election will be held in " + multi.master_timeout_sec + " seconds");
