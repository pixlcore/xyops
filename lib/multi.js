// xyOps Multi-Master Management Layer
...
class Multi {
		...
		async.each(
			...
			function() {
				...
				if (winner == self.hostID) {
					...
					// log transaction
					self.logTransaction('master_primary', "Master server is now primary: " + self.hostID, { host: self.hostID });
				}
				else {
	...
	receiveNewMaster(socket, data) {
		...
		if (fs.existsSync(recovery_file)) {
			...
			catch(err) { this.logError('master', "Failed to delete recovery file: " + recovery_file + ": " + err); }
		}
	}
