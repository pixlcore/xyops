...
class Jobs {
	...
	launchJob(job, callback) {
		...
		if (plugin) {
			...
			// allow specific admin-locked plugin params to override uid/gid
			var uid_param = Tools.findObject( plugin.params || [], { id: 'uid' } );
			if (uid_param && uid_param.locked && job.params && ('uid' in job.params) && String(job.params.uid).length) {
				job.uid = String(job.params.uid);
			}
			var gid_param = Tools.findObject( plugin.params || [], { id: 'gid' } );
			if (gid_param && gid_param.locked && job.params && ('gid' in job.params) && String(job.params.gid).length) {
				job.gid = String(job.params.gid);
			}
		} // plugin
