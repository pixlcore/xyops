...
class Jobs {
	...
	launchJob(job, callback) {
		...
		this.appendMetaLog(job, `OpsRocket job starting: #${job.id}` );
	...
	checkJobStartLimits(job) {
		...
		if (file_limit && job.input && job.input.files) {
			if (file_limit.amount && (job.input.files.length > file_limit.amount)) {
				this.abortJob(job, `Maximum number of allowed input files has been exceeded (${file_limit.amount}).`);
				return false;
			}
	...
	finishJob(job) {
		...
		job.actions = job.actions.concat( this.config.getPath('job_universal_actions.' + temp_job_type) );
