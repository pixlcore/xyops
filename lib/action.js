...
class Actions {
		...
		async.parallel(
			[
				...
				function(callback) {
					...
					self.logDebug(9, "Sending notification to users for " + action.trigger, channel.users);
	...
	sendEmail(text, args, callback) {
		...
		if (max_emails_per_day && (this.getDailyStat('email_sent') >= max_emails_per_day)) {
			return callback( new Error(`Maximum emails per day limit has been reached (${max_emails_per_day})`) );
		}
