...
class Workflows {
	...
	startWorkflow(job) {
		...
		// workflow itself may have actual targets specified (i.e. set from a parent workflow)
		// allow wf targets to override all node targets, but _xy_override_targets will still prevail if set
		if (job.targets && job.targets.length) {
			this.logWorkflow(job, null, `Overriding all node targets with: ` + job.targets.map( target => '#' + target ).join(', '));
			workflow.nodes.forEach( function(node) {
				if ((node.type == 'event') || (node.type == 'job')) {
					node.data.targets = Tools.copyHash( job.targets, true );
				}
			} );
		}
	...
	runWFNode_event(opts) {
		...
		// enforce max jobs per workflow run
		var max_jobs = this.config.get('max_jobs_per_workflow') || 1000;
		if (this.countTotalWFJobs(job) >= max_jobs) {
			return this.abortJob(job, `Maximum jobs per workflow reached (${max_jobs})`);
		}
		
		// if node is set to replay, do that now
		if (node.data.replay) {
			return this.replayWorkflowJob(opts);
		}
		...
		if (job.test) {
			new_job.test = true;
			['test_actions', 'test_limits', 'test_conditions'].forEach( function(key) {
		...
		// pass workflow-level _xy_override_ params into sub-job props
		if (job.params) {
			for (var key in job.params) {
				if (key.match(/^_xy_override_(\w+)$/) && job.params[key] && job.params[key].length) {
					...
					if (self.isReservedJobOverrideKey(key)) {
						self.logWorkflow(job, node.id, "Ignoring reserved job override param: " + key);
						continue;
					}
					new_job[sub_key] = job.params[key];
				}
			} // foreach param
		} // job.params
	...
	runWFNode_job(opts) {
		...
		this.logWorkflow(job, node.id, `Running custom job: ${new_job.plabel || plugin.title}`, { job: new_job });
	...
	runWFNode_action(opts) {
		...
		// possibly skip due to test conditions
		if (job.test && job.test_conditions && !job.test_conditions.includes(opts.condition)) {
			this.logWorkflow(job, node.id, `Skipping ${opts.condition} action due to test conditions: ${node.data.type}`);
			return;
		}
	...
	runWFController_multiplex(opts) {
		...
		if (!targets.length) {
			state.error = true;
			return this.abortJob(job, `No targets found for multiplex controller.`);
		}
	...
	runWFController_decision(opts) {
		...
		var context = Object.assign( {}, opts.context || opts.overrides.input );
		...
		if (!context.input) context.input = context; // convenience
		if (!context.workflow) context.workflow = job;
		if (!context.params) context.params = job.params || {};
		if (!context.workflowData) context.workflowData = details.workflowData;
		...
		if (!value) {
			...
			if (node.data.abort) {
				this.abortJob(job, `Decision expression evaluated to false: ${exp}`);
			}
	...
	replayWorkflowJob(opts) {
		// replay old sub-job and infuse results into workflow
		var self = this;
		var { job, node, source } = opts;
		
		this.logWorkflow(job, node.id, `Replaying job: #${node.data.replay}`);
		
		this.unbase.get( 'jobs', node.data.replay, function(err, sub_job) {
			if (job.complete) return; // sanity
			if (err) {
				return self.abortJob(job, "Job not found for replay: " + node.data.replay);
			}
			
			// clean up job data for replay
			sub_job.replay = true;
			sub_job.source = 'workflow';
			
			if (!sub_job.workflow) sub_job.workflow = {};
			sub_job.workflow.job = job.id;
			sub_job.workflow.node = node.id;
			sub_job.workflow.launcher = source ? source.id : null;
			delete sub_job.workflow.redirect;
			
			self.finishWorkflowJob(sub_job);
		}); // unbase.get
	}
	...
	finishWorkflowJob(sub_job) {
		...
		if (sub_job.replay) stub.replay = sub_job.replay;
		...
		if (sub_job.workflow.redirect) {
			// special redirect to custom node (i.e. from suspend/resume action)
			var dest = Tools.findObject( workflow.nodes, { id: sub_job.workflow.redirect } );
			if (dest) {
				self.logWorkflow(job, node.id, `Launching ${dest.type} node #${dest.id} via user resume redirect.`);
				self.runWorkflowNode({
					job: job,
					node: dest,
				...
				// should never happen
				return this.abortJob(job, "Workflow user resume redirect node not found: " + sub_job.workflow.redirect);
			}
		}
		else {
	...
	updateWorkflowController(opts) {
		...
		if (!state.active) {
			// can happen if a user does a resume jump to a controlled node
			this.logWorkflow(job, node.id, `WARNING: Controller is no longer active, skipping update from job #` + sub_job.id);
			return;
		}
		opts.state = state;
	...
	tickWorkflow(job) {
		...
		if (!workflow.state) state = workflow.state = {};
	...
	tickWFController_join(opts) {
		// tick join controller
		// detect idle state (no jobs or wait controllers active)
		var self = this;
		var { job, node, state } = opts;
		var workflow = job.workflow;
		var details = this.jobDetails[job.id];
		
		if (!state.active) return; // sanity
		...
		if (num_active_jobs) return;
		
		// now check for any active "wait" controllers, as they may be a prelude to a job
		var num_active_waits = 0;
		
		for (var key in workflow.state) {
			if (workflow.state[key].active) {
				var temp_node = Tools.findObject( workflow.nodes, { id: key } );
				if (temp_node && (temp_node.type == 'controller') && (temp_node.data.controller == 'wait')) num_active_waits++;
			}
		}
		
		// stuff waiting on other stuff
		if (num_active_waits) return;
		
		// workflow appears to be idle, so deactivate ourselves to allow flow to complete
		this.logWorkflow(job, node.id, `No active jobs or wait controllers, so deactivating join.`);
		state.active = false;
	}
	
	countTotalWFJobs(job) {
		// count total running and completed workflow sub-jobs
		var count = this.findActiveJobsDeep({ 'workflow.job': job.id }).length;
		
		if (job.workflow.jobs) {
			for (var node_id in job.workflow.jobs) {
				count += job.workflow.jobs[node_id].length || 0;
			}
		}
