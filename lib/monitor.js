...
class Monitoring {
		...
		monitor_defs.forEach( function(mon_def) {
			...
			if (mon_def.data_match) {
				...
				if (matches && (matches.length >= 2)) {
					...
					self.logDebug(7, "Custom data regular expression did not match", {
						monitor: mon_def,
			...
			[
				...
				function(callback) {
					...
					alert_defs.forEach( function(alert_def) {
						...
						self.logDebug(9, "Checking alert expression for " + params.hostname + '/' + alert_def.id + ": " + exp, {
							alert: alert_def,
							server: server.id,
							hostname: params.hostname,
							expression: exp
					...
					Object.keys(params.alerts).forEach( function(key) {
						...
						if ((alert.count == alert_def.samples) && !alert.notified) {
							...
							(server.groups || []).forEach( function(group) {
								self.updateDailyCustomStat( `groups.${group.id}.alert_new`, 1 );
							} );
							...
							(server.groups || []).forEach( function(group) {
								self.updateDailyCustomStat( `groups.${group.id}.alert_cleared`, 1 );
							} );
	...
	sendAlertNotification(args) {
		...
		if (alert_def.channel_id && (args.template == 'alert_new')) {
			...
			if (!channel) {
				this.logDebug(9, "Notification Channel not found for alert: " + alert_def.title + ": " + alert_def.channel_id );
			}
			else if (!channel.enabled) {
				this.logDebug(9, "Notification Channel not enabled for alert: " + alert_def.title + ": " + alert_def.channel_id );
			}
			else if (channel.max_per_day && (this.getDailyCustomStat(`channels.${channel.id}.invocations`) >= channel.max_per_day)) {
				this.logDebug(9, `Notification Channel has reached maximum daily limit: ${channel.id} (${channel.max_per_day}/day)`);
			}
			else {
	...
	createGroupSnapshot(group_id, params, callback) {
		...
		this.logDebug(6, "Taking snapshot of group: " + group_id + " (" + servers.length + " servers)", params);
		...
		this.storage.getMulti( host_keys, function(err, values) {
			...
			self.logDebug(7, "Saving snapshot of group: " + group.id, { servers: servers.length, snapshot_id: snapshot.id });
	...
	checkGroupWatches() {
		...
		async.eachSeries( snap_groups,
			...
			function(err) {
				...
				else self.logDebug(7, "Group watch snaps complete");
			}
		);
	}
	...
	handleQuickMonData(socket, data) {
		...
		if (list.length && (list[list.length - 1].date > stub.date)) {
			this.logDebug(9, "Dropping QuickMon sample due to clock conflict", { server: server.id, last: list[list.length - 1].date, current: stub.date });
			return;
		}
