...
class Monitoring {
	...
	setupMonitoring(callback) {
		...
		this.unbase.search( 'alerts', 'active:true', {}, function(err, data) {
			...
			data.records.forEach( function(alert) {
				var global_id = alert.server + '-' + alert.alert;
				...
				function(callback) {
					...
					alert_defs.forEach( function(alert_def) {
						...
						try { 
							...
							self.logMonitor(9, "Expression evaluated to: " + result, { server: server.id, alert: alert_def.id, expression: alert_def.expression });
						}
						catch (err) {
					...
					Object.values(self.warmAlerts).forEach( function(alert) {
						if (alert.server != server.id) return;
						
						var alert_def = Tools.findObject( alert_defs, { id: alert.alert } );
						...
						if ((alert.count == alert_def.samples) && !alert.notified) {
							...
							params.new_alerts[alert_def.id] = true;
						...
						alert.modified = params.date;
						
						// cleanup dead alerts
						if (!alert.count) {
							delete self.activeAlerts[ global_id ];
							delete self.warmAlerts[ global_id ];
							alerts_changed = true;
						}
					}); // foreach alert
	...
	findWarmAlerts(criteria) {
		// find warm alerts matching criteria -- return array
		return Tools.findObjects( Object.values(this.warmAlerts), criteria );
	}
	
	clearActiveAlerts(criteria) {
		// clear multiple active alerts by criteria search
		var self = this;
		
		this.findActiveAlerts(criteria).forEach( function(alert) {
			self.clearActiveAlert( alert.id );
		} );
		
		// also delete warms
		Tools.deleteObjects( Object.values(this.warmAlerts), criteria );
	}
	
	clearActiveAlert(id) {
		// manually clear active alert by id (i.e. from job)
		var self = this;
		var now = Tools.timeNow(true);
		
		var global_alert = Tools.findObject( Object.values(this.activeAlerts), { id } ) || this.activeAlerts[id];
		if (!global_alert) {
			this.logMonitor(6, "Alert not found to clear: " + id);
			return;
		}
		
		this.logMonitor(6, "Manually clearing active alert: " + id, global_alert);
		
		var server_id = global_alert.server;
		var alert_def_id = global_alert.alert;
		var global_id = server_id + '-' + alert_def_id;
		
		var alert_def = Tools.findObject( this.alerts, { id: alert_def_id } );
		if (!alert_def) {
			this.logMonitor(6, "Alert definition not found for clear: " + alert_def_id);
			return;
		}
		
		var server = this.servers[server_id];
		if (!server) {
			this.logMonitor(6, "Server not found for alert clear: " + server_id);
			return;
		}
		
		var host_key = 'hosts/' + server.id + '/data';
		
		this.storage.get( host_key, function(err, params) {
			if (err) {
				self.logMonitor(6, "Failed to load server data for alert clear: " + host_key + ": " + err);
				return;
			}
			...
			self.logTransaction('alert_cleared', "Alert cleared: " + server.hostname + ": " + alert_def.title, { 
				server: server.id,
				hostname: server.hostname,
				ip: server.ip,
				alert: global_alert
			...
			self.unbase.update( 'alerts', global_alert.id, { active: false, modified: now }, function(err) {
				if (err) self.logError('db', "Failed to update alerts DB: " + global_alert.id + ": " + err);
				if (alert_def.enabled) self.sendAlertNotification({
					template: "alert_cleared",
					alert_def: alert_def,
					params: params,
					server: server,
					global_alert: global_alert,
					elapsed: now - global_alert.date
				});
			} );
			...
			self.doUserBroadcastAll('update', { activeAlerts: self.activeAlerts });
			self.logMonitor(6, "Alert has been manually cleared: " + id);
		}); // storage.get
	}
	...
	cleanupGlobalAlerts() {
		...
		var now = Tools.timeNow(true);
		var alerts_changed = false;
		var stale_alert_timeout = this.config.get('stale_alert_timeout') || 90;
		
		for (var key in this.activeAlerts) {
			var alert = this.activeAlerts[key];
			if (now - alert.modified > stale_alert_timeout) {
				this.logMonitor(9, "Clearing stale alert: " + key);
		...
		// expire warm alerts too, but silently
		for (var key in this.warmAlerts) {
			var alert = this.warmAlerts[key];
			if (now - alert.modified > stale_alert_timeout) {
				delete this.warmAlerts[key];
			}
		}
